...
def _run_repair_prompt(runner: Any, context: RepairContext, prior_attempts: list[RepairAttemptSummary]) -> str:
    ...
    payload = {"model": runner.model, "messages": call_messages, "system": build_system_blocks(runner.repo), "tools": tool_specs(), "max_tokens": runner.max_tokens, "stream": False}
    raw = runner._observed_model_call(payload, stream=False) if hasattr(runner, "_observed_model_call") else runner.client.create_message(payload, stream=False)
