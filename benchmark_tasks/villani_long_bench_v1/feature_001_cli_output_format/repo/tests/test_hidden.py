...
def test_csv_format_is_supported_programmatically():
    assert run_stats([2, 4], 'csv') == 'count,total,average\n2,6,3.0'
...
def test_csv_uses_canonical_field_order_even_for_reordered_input():
    reordered = {'total': 6, 'average': 3.0, 'count': 2}
    assert format_summary(reordered, 'csv') == 'count,total,average\n2,6,3.0'
