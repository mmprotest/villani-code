...
import { openAIChatToPiContext, piAssistantToOpenAIResponse } from "./modelProxy.js";
...
test("proxy serves minimal OpenAI chat completions without credentials", async () => {
  const model = {
    id: "pi-test",
    name: "Pi Test",
    api: "openai-completions",
  ...
  const proxy = new PiModelProxy({
    ...
    completeFn: async (_model, context) => ({
      ...
      provider: "pi",
      model: "pi-test",
      content: [{ type: "text", text: `saw ${context.messages.length} message` }],
