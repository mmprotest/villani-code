...
def test_benchmark_runner_aggregates_runtime_usage_into_result(tmp_path: Path, monkeypatch) -> None:
    from villani_code.benchmark.adapters.base import AdapterEvent, AdapterRunResult
    from villani_code.benchmark.models import FairnessClassification, FieldQuality, TelemetryQuality
    ...
    class FakeAgent:
        ...
        def run_agent(self, **kwargs):
            return AdapterRunResult(
                ...
                telemetry_field_quality_map={
                    "num_shell_commands": FieldQuality.EXACT,
                ...
                events=[
                    AdapterEvent(
                        ...
                        timestamp=1.0,
                        payload={"type": "model_request_finished", "usage": {"input_tokens": 10, "output_tokens": 4}},
                    ),
                    AdapterEvent(
                        type="model_request_finished",
                        timestamp=2.0,
                        payload={"type": "model_request_finished", "usage": {"input_tokens": 6, "output_tokens": 3, "total_tokens": 9}},
                    ),
                ],
            )
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (True, [], 1.0, 2.0, False),
    )
    monkeypatch.setattr("villani_code.benchmark.runner.list_touched_files", lambda _repo: ["src/app.py"])
    monkeypatch.setattr("villani_code.benchmark.runner.line_stats", lambda _repo: (1, 0))
