...
def test_ordinary_success_emits_required_artifacts_and_exact_usage(tmp_path: Path) -> None:
    ...
    assert set(trajectory) == {"schema_version", "session_id", "agent", "steps", "final_metrics", "extra"}
    ...
    assert {step["source"] for step in steps} <= {"system", "user", "agent"}
    assert all("message" in step for step in steps)
    assert trajectory["final_metrics"] == {"total_prompt_tokens": 8, "total_completion_tokens": 10, "total_steps": len(steps)}
    response_steps = [step for step in steps if step.get("metrics")]
    assert response_steps[0]["metrics"] == {"prompt_tokens": 3, "completion_tokens": 4}
    tool_steps = [step for step in steps if step.get("tool_calls")]
    ...
    exported_call_ids = {call["tool_call_id"] for step in tool_steps for call in step["tool_calls"]}
    assert {result["source_call_id"] for result in tool_step["observation"]["results"]} <= exported_call_ids
...
def test_timeout_path_emits_partial_artifacts(tmp_path: Path) -> None:
    ...
    run_dir = _mission_dir(tmp_path)
    assert (run_dir / "full_transcript.json").exists()
    trajectory = _load(run_dir / "trajectory.json")
    assert trajectory["schema_version"] == "ATIF-v1.7"
    assert trajectory["extra"]["verified_outcome"] == "timed_out"
    assert trajectory["final_metrics"]["total_steps"] == len(trajectory["steps"])
    assert all(isinstance(step["step_id"], int) and step["source"] in {"system", "user", "agent"} and "message" in step for step in trajectory["steps"])
...
def test_model_exception_path_emits_exception_artifacts(tmp_path: Path) -> None:
    ...
    run_dir = _mission_dir(tmp_path)
    assert _load(run_dir / "full_transcript.json")["events"][-1]["type"] == "terminal_state"
    trajectory = _load(run_dir / "trajectory.json")
    assert trajectory["extra"]["verified_outcome"] == "exception"
    assert trajectory["final_metrics"]["total_steps"] == len(trajectory["steps"])
    assert all(step["source"] in {"system", "user", "agent"} and "message" in step for step in trajectory["steps"])
...
def test_meaningful_repo_edit_tracking_matches_pre_telemetry_placement() -> None:
    state_source = Path("villani_code/state.py").read_text(encoding="utf-8")
    expected = 'if tool_name in {"Write", "Patch"} and not result.get("is_error"):\n                    targets = self._extract_tool_targets(tool_name, tool_input)\n                    if any(not _is_generated_or_runtime_artifact(target) for target in targets):\n                        meaningful_repo_edit_made = True\n                        prose_edit_intent_recovery_attempts = 0\n                continue'
    assert expected in state_source
